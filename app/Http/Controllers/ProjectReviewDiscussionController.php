...
use App\Models\ProjectReviewDiscussionReply;
...
class ProjectReviewDiscussionController extends Controller
{
    private const REVIEW_MODEL_MAP = [
        'concept_design_review' => ConceptDesignReview::class,
        ...
        'leed_review' => LeedReview::class,
    ];

    public function index(Request $request, $reviewType, $reviewId)
    {
        $review = $this->findReview($reviewType, $reviewId);
        if (! $review) {
            return $this->returnErrorData('ไม่พบข้อมูล', 404);
        }

        $topics = ProjectReviewDiscussionTopic::query()
            ->where('review_type', $reviewType)
            ->where('review_id', $review->getKey())
            ->whereNull('deleted_at')
            ->when($request->filled('document_key'), function ($query) use ($request) {
                $query->where('document_key', $request->input('document_key'));
            })
            ->with(['replies'])
            ->orderBy('topic_no')
            ->orderBy('id')
    ...
    public function storeTopic(Request $request, $reviewType, $reviewId)
    {
        $review = $this->findReview($reviewType, $reviewId);
        if (! $review) {
            return $this->returnErrorData('ไม่พบข้อมูล', 404);
        ...
        try {
            $topic = new ProjectReviewDiscussionTopic();
            $topic->review_type = $reviewType;
            $topic->review_id = $review->getKey();
            $topic->document_key = $this->nullableString($request->input('document_key'));
            $topic->document_label = $this->nullableString($request->input('document_label'));
            $topic->topic_no = $this->nextTopicNumber($reviewType, (int) $review->getKey());
            ...
            $topic->workflow_role = $this->nullableString($request->input('workflow_role'));
    ...
    public function updateTopic(Request $request, $topicId)
    {
        ...
        try {
            ...
            if ($request->has('document_key')) {
                $topic->document_key = $this->nullableString($request->input('document_key'));
            }

            if ($request->has('document_label')) {
                $topic->document_label = $this->nullableString($request->input('document_label'));
            }
    ...
    public function deleteTopic(Request $request, $topicId)
    {
        ...
        try {
            ...
            ProjectReviewDiscussionReply::query()
                ->where('topic_id', $topic->id)
                ->whereNull('deleted_at')
                ->get()
                ->each(function (ProjectReviewDiscussionReply $reply) use ($actorId) {
    ...
    public function storeReply(Request $request, $topicId)
    {
        ...
        try {
            $reply = new ProjectReviewDiscussionReply();
            ...
            $reply->workflow_role = $this->nullableString($request->input('workflow_role'));
    ...
    {
        $reply = ProjectReviewDiscussionReply::query()
            ->where('id', $replyId)
            ->whereNull('deleted_at')
            ->first();
    ...
    protected function findReview(string $reviewType, $reviewId): ?Model
    {
        $modelClass = self::REVIEW_MODEL_MAP[$reviewType] ?? null;
        if (! $modelClass) {
            return null;
        }
    ...
    protected function findTopic($topicId, bool $withReplies = false): ?ProjectReviewDiscussionTopic
    {
        $query = ProjectReviewDiscussionTopic::query()
            ->where('id', $topicId)
            ->whereNull('deleted_at');
    ...
    protected function nextTopicNumber(string $reviewType, int $reviewId): int
    {
        $currentMax = (int) ProjectReviewDiscussionTopic::query()
            ->where('review_type', $reviewType)
    ...
    protected function transformTopic(ProjectReviewDiscussionTopic $topic): array
    {
        ...
        return [
            ...
            'review_id' => (int) $topic->review_id,
            'document_key' => $topic->document_key,
            'document_label' => $topic->document_label,
            ...
            'workflow_role' => $topic->workflow_role,
    ...
    protected function transformReply(ProjectReviewDiscussionReply $reply): array
    {
        return [
            'id' => $reply->id,
            ...
            'workflow_role' => $reply->workflow_role,
