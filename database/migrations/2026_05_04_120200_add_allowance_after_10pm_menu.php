...
class AddAllowanceAfter10pmMenu extends Migration
{
    public function up()
    {
        ...
        $existingMenu = DB::table('menus')
            ->where('key', 'mm2.allowance_after_10pm')
            ->first();
        ...
        if (Schema::hasColumn('menus', 'key')) {
            $menuData['key'] = 'mm2.allowance_after_10pm';
        }
        if (Schema::hasColumn('menus', 'path')) {
            $menuData['path'] = '/allowance-after-10pm';
        }
    ...
    public function down()
    {
        ...
        $menu = DB::table('menus')->where('key', 'mm2.allowance_after_10pm')->first();
        if (!$menu) {
            return;
        }
